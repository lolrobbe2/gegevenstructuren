...
public class SkipListLane{
    ...
    public LayerNode insert(int key) {
        if(size == 0){
        ...
        if(key < previous.key){
            previous.left = new LayerNode(key,current,previous,null);//TODO bottom (bottom is set by next lane)
            size++;
            return previous.left;
        } else {
            previous.right = new LayerNode(key, previous,current,null);//TODO bottom (bottom is set by next lane)
            size++;
